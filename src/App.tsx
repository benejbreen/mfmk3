...
export default function MyFirstMerchForm() {
  ...
  const handleSizeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newSizeCounts = { ...sizeCounts, [name as keyof typeof sizeCounts]: numValue };
    const total = Object.values(newSizeCounts).reduce((acc, val) => acc + val, 0);
    if (total <= 50) {
      setSizeCounts(newSizeCounts);
    }
  };

  const totalCount = Object.values(sizeCounts).reduce((acc, val) => acc + val, 0);
